-- Migration: Add AI curriculum generation fields
...
-- Add constraints for approval_status
ALTER TABLE public.curricula ADD CONSTRAINT IF NOT EXISTS check_approval_status 
  CHECK (approval_status IN ('pending', 'approved', 'rejected'));

-- Add constraints for curriculum_type
ALTER TABLE public.curricula ADD CONSTRAINT IF NOT EXISTS check_curriculum_type 
  CHECK (curriculum_type IN ('crash_course', 'standard', 'comprehensive', 'mastery'));

-- Add constraints for content_density
ALTER TABLE public.learning_sessions ADD CONSTRAINT IF NOT EXISTS check_content_density 
  CHECK (content_density IN ('light', 'moderate', 'intensive'));

-- Add constraints for session_type
ALTER TABLE public.learning_sessions ADD CONSTRAINT IF NOT EXISTS check_session_type 
